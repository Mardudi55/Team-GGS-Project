...
public class AccessLayerFacade {
    ...
    public List<BlockData> fetchLatestBlocks(int count) {
        ...
        return blockFetcher.fetchLatestBlocks(count);
    }

    public List<TransactionData> fetchTransactions(BlockData block) {
        try {
            rateLimitHandler.acquire();
        } catch (InterruptedException e) {
//            e.printStackTrace();
        ...
        return transactionFetcher.fetchTransactions(block);
    }
